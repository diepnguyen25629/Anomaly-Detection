...
#define MAX_SEND_IMUPT 50
#define MAX_SEND_AUDIO 8
...
{   
    ...
    const TickType_t imu_interval_ticks = pdMS_TO_TICKS(1000 / mpu6500_cfg.imu_sample_rate);
    const TickType_t pre_interval_ticks = pdMS_TO_TICKS(1000 / bmp280_cfg.pre_sample_rate);
    const TickType_t temp_interval_ticks = pdMS_TO_TICKS(1000 / ds18b20_cfg.temp_sample_rate);

    // Tick đếm cho từng cảm biến
    TickType_t last_imu_tick = xTaskGetTickCount();
    TickType_t last_pre_tick = last_imu_tick;
    TickType_t last_temp_tick = last_imu_tick;

    // Dùng vTaskDelayUntil theo tick nhỏ nhất
    const TickType_t base_interval = MIN(MIN(imu_interval_ticks, pre_interval_ticks), temp_interval_ticks);
    ...
    while (1)
    {
        ...
        // Chạy vòng lặp đến khi đến chu kì lấy mẫu của cảm biến mới cập nhật dữ liệu 
        if ((now - last_imu_tick) >= imu_interval_ticks)
        {
            last_imu_tick = now;
        ...
        if ((now - last_pre_tick) >= pre_interval_ticks)
        {
            last_pre_tick = now;
        ...
        if ((now - last_temp_tick) >= temp_interval_ticks)
        {
            last_temp_tick = now;
        ...
        // Dữ liệu cập nhật được đưa vào hàng đợi 
        if (data_updated) {
            ...
            if(sd_record_queue != NULL) {
                if (xQueueSend(sd_record_queue, &packet, 0) != pdTRUE) {
                    ESP_LOGE(TAG, " imupt full sd_record_queue");
                }
            }
        ...
        vTaskDelayUntil(&xLastWakeTime, base_interval);
    }
}
...
void read_audio_task(void *pvParameter)
{
    ...
    while (1) {
        ...
        if (bytes > 0) {
            ...
            if (sd_record_queue != NULL) {
                if( xQueueSend(sd_record_queue, &packet, 0) != pdTRUE) {
                    ESP_LOGE(TAG,"audio full sd_record_queue");
                }
            }
...
void write_data_to_sd_task(void *pvParameter) {
    ...
    const int64_t REOPEN_INTERVAL_US = 60 * 1000 * 1000;
    ...
    while(1) {
        ...
        // Đóng mở file 1 phút 1 lần tránh mất dữ liệu 
        if (data_file != NULL && now - last_reopen_us > REOPEN_INTERVAL_US) {
            ESP_LOGI(TAG, "Cycling file: flushing to prevent data loss");
        ...
        if (is_recording && data_file != NULL) {
            if (xQueueReceive(sd_record_queue, &recv_data, portMAX_DELAY) == pdTRUE) {
                if (recv_data.type == PACKET_TYPE_IMUPT) {
                    ...
                    if (imupt_packet.valid_count >= MAX_SEND_IMUPT) {
                        int send_size = sizeof(uint16_t) * 3 + imupt_packet.valid_count * sizeof(audio_data_t);
                        sd_spi_write_binary_buffer(data_file, &imupt_packet, send_size, 1);
                    ...
                    if (audio_packet.valid_count >= MAX_SEND_AUDIO) {
                        ...
                        sd_spi_write_binary_buffer(data_file, &audio_packet, send_size, 1);
                        vTaskDelay(pdMS_TO_TICKS(2));
...
void bt_wifi_config_task(void *pvParameter)
{
    ...
    is_connect_gui = true;   
    gui_send_queue = xQueueCreate(64, sizeof(data_packet_t ));
...
void app_main(void)
{
    ...
    sd_record_queue = xQueueCreate(50, sizeof(data_packet_t));
    if (!sd_record_queue) {
    ...
    xTaskCreate(write_data_to_sd_task, "WriteIMUPT", 8192, NULL, 6, NULL);
